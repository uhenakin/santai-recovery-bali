...
export default function HotPoolPage() {
  ...
  // Path sudah menggunakan .webp dan 9 file berurutan
  const galleryImages = [1, 2, 3, 4, 5, 6, 7, 8, 9].map((n) => `/images/hotpool/hotpool${n}.webp`);
    ...
    <main className="w-full bg-[#000000] text-white">
      ...
      <section className="relative w-full min-h-[65vh] md:min-h-[70vh] flex flex-col justify-center pt-32 pb-16">
        <div className="absolute inset-0 z-0">
          <Image src="/images/bg/bg-hotpool.webp" alt="Hot Pool" fill sizes="(max-width: 768px) 85vw, 33vw" className="object-cover object-center" priority />
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-8 w-full flex flex-col items-start drop-shadow-2xl mt-4">
          <p className="text-[11px] tracking-[0.4em] uppercase text-[#D4AF37] mb-4 font-bold">Warm Immersion</p>
          <h1 className="font-serif text-5xl md:text-7xl mb-6 md:mb-8 leading-tight">
            Hot <span className="text-[#D4AF37] italic">Pool</span>
          </h1>
          <div className="space-y-4 md:space-y-6 text-white text-sm md:text-base font-light leading-relaxed max-w-2xl font-medium">
            <p>Experience pure relaxation in our therapeutic hot pool. The warm water gracefully relieves joint pressure, eases muscle stiffness, and significantly improves blood flow throughout your entire body.</p>
            <p>Perfect for unwinding after a long day or intense physical exertion. The sustained warmth helps lower stress levels, creating a deep sense of tranquility and physical release.</p>
          </div>
          <div className="mt-8 md:mt-12 flex gap-8 pt-6 md:pt-8 w-full max-w-xl">
            <div>
              ...
              <p className="font-serif text-2xl text-white">38°C - 40°C</p>
            </div>
            <div>
              <p className="text-[#D4AF37] text-[10px] tracking-widest uppercase font-bold mb-1">Duration</p>
              <p className="font-serif text-2xl text-white">15 - 20 Mins</p>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="flex touch-pan-y -ml-4">
            {galleryImages.map((img, i) => (
              ...
              >
                ...
                >
                  <Image src={img} alt={`Hot Pool ${i + 1}`} fill sizes="(max-width: 768px) 85vw, 33vw" className="object-cover group-hover:scale-105 transition-transform duration-500" />
